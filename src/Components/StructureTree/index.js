...
const StyledTreeItem = (props) => {
    ...
    return (<TreeItem {...props} label={
        <Box>
            <Checkbox className="checkbox" checked={itemChecked(status)}/>
            {status}
            {labelText}
        </Box>
    } ></TreeItem>);
    ...
    function GetNode(nodes) {
        if (nodes !== undefined) {
            return (nodes.map(x => {
                console.log(x);
                return (
                    <StyledTreeItem className="styledTreeItem" nodeId={x.elementId.toString()} key={x.elementId} labelText={x.name} status={x.status}>
                        {GetNode(x.elements)}
                    </StyledTreeItem>
                )
            })
